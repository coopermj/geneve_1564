...
print("integration: generate Matthew from cache")
import subprocess as _subprocess, tempfile as _tempfile

_root = os.path.join(os.path.dirname(__file__), "..")


def _generate_book_tex(book_dir):
    tmp = _tempfile.mkdtemp(prefix="net_rl_")
    _subprocess.run(
        [sys.executable, os.path.join(_root, "scripts", "generate_bible.py"),
         "--books", book_dir, "--output-dir", tmp,
         "--cache-dir", os.path.join(_root, "data", "net_bible_cache")],
        check=True, capture_output=True, text=True,
    )
    with open(os.path.join(tmp, book_dir, f"{book_dir}.tex"), encoding="utf-8") as fh:
        return fh.read()


# Matthew is fully cached (28 chapters, incl. the Sermon on the Mount).
tex = _generate_book_tex("matthew")
on = tex.count("\\redletteron")
off = tex.count("\\redletteroff")
check("matthew: emits red letter", on > 50, f"on={on}")
check("matthew: toggles balanced", on == off, f"on={on} off={off}")
