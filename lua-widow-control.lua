...
-- PATCH (geneve_1564, 2026-07-17): ids for nodes that must be transparent
-- when scanning back for a heading penalty (see move_last_line).
local mark_id = id_from_name("mark")
local whatsit_id = id_from_name("whatsit")
local kern_id = id_from_name("kern")
...
local function first_last_paragraphs(head)
    ...
    -- PATCH (geneve_1564, 2026-07-17): find_attribute returns nil when no
    -- (further) node on the page carries the paragraph attribute -- e.g. a
    -- column whose every paragraph began on the previous page. Guard the
    -- loop and bail out instead of crashing on abs(nil).
    while first_val and abs(first_val) // PAGE_MULTIPLE == pagenum - 1 do
    ...
    if not first_val or not last_index then
        return nil, nil    -- PATCH (geneve_1564): no expandable paragraph here
    end
...
local function best_paragraph(head)
    ...
    if not first_paragraph_index or not last_paragraph_index then
        return nil    -- PATCH (geneve_1564): nothing on this page to expand
    end
...
local function move_last_line(head)
    ...
    while n do
        ...
        elseif n.id == mark_id or n.id == whatsit_id or n.id == kern_id then
            -- PATCH (geneve_1564, 2026-07-17): marks (e.g. per-verse
            -- \markboth) and whatsits sit between a heading's \nobreak
            -- penalty and the following line; treating them as "not found"
            -- made lwc move the lone line and ORPHAN the heading. They are
            -- transparent for break analysis -- skip them like glue.
    ...
    -- PATCH (geneve_1564, 2026-07-17): the walk above can leave `n` pointing
    -- at a trailing non-line node (e.g. a mark node from a per-verse
    -- \markboth, or a whatsit); the code below assumes `n` is the last
    -- line's hlist and crashes with "You cannot set field list in a node of
    -- type mark". Skip back to the last hlist; bail out gracefully if the
    -- page has none.
    while n and n.id ~= hlist_id do
        n = n.prev
    end
    if not n or not n.prev or not n.prev.prev then
        return false
    end
